...
%vector quantization
[m, p, distor] = vqsplit([
    circ_gest1; circ_gest2; circ_gest3; %{circ_gest4; circ_gest5; circ_gest6; circ_gest7; circ_gest8; circ_gest9;
    %}
    ]', codebookSize);
...
% %{
idx = knnsearch(md1, circ_gest1); % match points with index
fprintf('Train %d:\n', count);
h.scaling = 1;
h.train_one(idx);
%}
...
for i = 1 : 10
    ...
    h.train_multiple(circ_idx1);
    %{
    h.train_multiple(circ_idx3);
    
    h.train_multiple(circ_idx4);
    
    h.train_multiple(circ_idx5);
    
    h.train_multiple(circ_idx6);
